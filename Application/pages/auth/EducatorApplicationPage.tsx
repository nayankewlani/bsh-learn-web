...
const EducatorApplicationPage: React.FC = () => {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: isDark
        ? "linear-gradient(135deg,#0A0000,#1A0008,#200010)"
      ...
      <button onClick={() => navigate(-1)} style={{ position: "fixed", top: 24, left: 24, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", color: "#FF6B8A", padding: "8px 16px", borderRadius: 50, display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 13 }}>
        <FiArrowLeft size={14} /> Back
      </button>

      <div style={{ width: "100%", maxWidth: 520 }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <img src={bshLogo} alt="BSH Healers" style={{ height: 56, objectFit: "contain", filter: "drop-shadow(0 10px 20px rgba(255,30,86,0.4))" }} />
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 24, padding: "36px 32px", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
          {submitted ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <button onClick={() => navigate("/login")} style={{ background: "linear-gradient(135deg,#FF1E56,#D4003F)", color: "#fff", border: "none", borderRadius: 12, padding: "13px 28px", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
                Back to Login
              </button>
            </div>
          ) : (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <HiOutlineAcademicCap size={26} color="#FF1E56" />
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    ...
                    <input value={form.name} onChange={set("name")} required placeholder="Your full name" style={inputStyle}
                      onFocus={(e) => { e.target.style.borderColor = "#FF1E56"; e.target.style.boxShadow = "0 0 0 3px rgba(255,30,86,0.1)"; }}
                ...
                <button type="submit" disabled={loading}
                  style={{ width: "100%", padding: 15, background: loading ? "#9ca3af" : "linear-gradient(135deg,#FF1E56,#D4003F)", color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  {loading
