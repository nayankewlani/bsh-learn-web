...
const LoginPage: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      {/* Decorative orbs */}
      <div style={{ position: "absolute", top: -80, left: -80, width: 300, height: 300, background: "radial-gradient(circle, rgba(255,30,86,0.15), transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <img src={bshLogo} alt="BSH Healers" style={{ height: 64, width: "auto", objectFit: "contain", filter: "drop-shadow(0 10px 20px rgba(255,30,86,0.4))", animation: "float 4s ease-in-out infinite" }} />
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 24, padding: "40px 36px", boxShadow: "0 25px 60px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 22 }}>
            {([
              ...
              <button key={r.key} type="button" onClick={() => setLoginAs(r.key)}
                style={{ padding: "11px 10px", borderRadius: 12, border: `2px solid ${loginAs === r.key ? "#FF1E56" : "#eee"}`, cursor: "pointer", background: loginAs === r.key ? "rgba(255,30,86,0.08)" : "#fafafa", color: loginAs === r.key ? "#FF1E56" : "#666", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontWeight: 600, fontSize: 13, transition: "0.2s" }}>
                {r.icon} {r.label}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                onFocus={(e) => { e.target.style.borderColor = "#FF1E56"; e.target.style.boxShadow = "0 0 0 4px rgba(255,30,86,0.12)"; }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#555", cursor: "pointer" }}>
                <input type="checkbox" style={{ accentColor: "#FF1E56" }} /> Remember me
              </label>
              <span onClick={() => navigate("/forgot-password")} style={{ color: "#FF1E56", fontWeight: 600, cursor: "pointer" }}>Forgot Password?</span>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              style={{ width: "100%", padding: 15, background: isLoading ? "#9ca3af" : "linear-gradient(135deg,#FF1E56,#D4003F)", color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: isLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "0.3s" }}
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button
              ...
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = "#FF1E56")}
          ...
          <p style={{ textAlign: "center", marginTop: 22, color: "#666", fontSize: 14 }}>
            ...
            <Link to="/register" style={{ color: "#FF1E56", fontWeight: 700, textDecoration: "none" }}>Sign up free</Link>
          </p>
        </div>
      </div>

      {toast && (
        <div style={{ position: "fixed", bottom: 32, left: "50%", transform: "translateX(-50%)", background: "#222222", color: "#FF6B8A", padding: "12px 22px", borderRadius: 50, fontSize: 13, fontWeight: 600, border: "1px solid rgba(255,30,86,0.4)", backdropFilter: "blur(12px)", zIndex: 9999, whiteSpace: "nowrap", boxShadow: "0 8px 24px rgba(0,0,0,0.4)", animation: "fadeIn 0.3s ease" }}>
          {toast}
        </div>
      )}
