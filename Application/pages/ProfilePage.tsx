  ...
  return (
    <div style={{ background: t.bgPrimary, minHeight: "100vh", padding: "32px 24px" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: t.textPrimary, marginBottom: 28 }}>My Profile</h1>
        <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 20, color: t.textPrimary }}>{user.name}</div>
              <div style={{ color: t.accentLight, fontSize: 14, marginTop: 2 }}>{user.role === "educator" ? "👨‍🏫 Educator" : "🎓 Student"}</div>
          ...
          {editing ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                <label style={{ color: t.textAccent, fontSize: 14, fontWeight: 500, display: "block", marginBottom: 6 }}>Bio</label>
                <textarea value={form.bio} onChange={(e) => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Tell students about yourself..." rows={3} style={{ width: "100%", background: t.bgInput, border: `1.5px solid ${t.borderLight}`, borderRadius: 10, color: t.textPrimary, padding: "10px 14px", fontSize: 15, resize: "vertical", boxSizing: "border-box", fontFamily: "inherit" }} />
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[["Email", user.email], ["Role", user.role], ["Phone", user.phone || "Not set"], ["Bio", user.bio || "No bio yet"]].map(([k, v]) => (
                <div key={k} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: `1px solid ${t.border}` }}>
                  <span style={{ color: t.textMuted, fontSize: 13, minWidth: 80 }}>{k}</span>
                  <span style={{ color: t.textAccent, fontSize: 14 }}>{v}</span>
                </div>
              ))}
